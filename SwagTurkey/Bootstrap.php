 ...
 */
class Shopware_Plugins_Core_SwagTurkey_Bootstrap extends Shopware_Components_Plugin_Bootstrap
{
    /**
     * {@inheritdoc}
     */
    ...
    public function install()
    {
        ...
        /** @var \Shopware\Models\Shop\Locale $turkishLocal */
        $turkishLocal = $localeRepository->findOneBy(['locale' => 'tr_TR']);
        if ($turkishLocal === null) {
            return parent::uninstall();
        }

        $turkishLocalId = $turkishLocal->getId();
        $locales = $element->getValue();

        if (!in_array($turkishLocalId, $locales)) {
            $locales[] = $turkishLocalId;
    ...
    public function uninstall()
    {
        ...
        if (($index = array_search($turkishLocalId, $locales)) !== false) {
            unset($locales[$index]);
            $element->setValue($locales);
            $modelManager->flush($element);
        }
