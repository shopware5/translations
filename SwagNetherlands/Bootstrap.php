...
class Shopware_Plugins_Core_SwagNetherlands_Bootstrap extends Shopware_Components_Plugin_Bootstrap
{
    /**
     * @var array
     */
    ...
    {
        return 'Netherlands Dutch translation';
    }
    ...
    public function uninstall()
    {
        ...
        $id = $localeRepository->findOneBy(['locale' => 'nl_NL'])->getId();
        ...
        foreach ($this->elementLabels as $elementLabel) {
            ...
            if (!empty($currentRow) && $currentRow['translations'] == 0) {
                Shopware()->Db()->insert('s_core_config_element_translations', [
                    ...
                    'description' => isset($elementLabel['description']) ? $elementLabel['description'] : null
                ]);
            }
        }
    }
    ...
    public function onDispatchLoopStartup()
    {
        // Register library dir
        Shopware()->Loader()->addIncludePath($this->Path() . 'Library/', Enlight_Loader::POSITION_PREPEND);
    }
